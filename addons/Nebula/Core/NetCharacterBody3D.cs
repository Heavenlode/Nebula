...
namespace Nebula
{
    /**
		<summary>
		<see cref="CharacterBody3D">CharacterBody3D</see>, extended with Nebula networking capabilities.
		See <see cref="NetNode"/> for more information.
		</summary>
	*/
    public partial class NetCharacterBody3D : CharacterBody3D, INetNode<NetCharacterBody3D>, INotifyPropertyChanged
    {
        public NetworkController Network { get; internal set; }
        public NetCharacterBody3D()
        {
        ...
        public static bool NetworkSerialize(WorldRunner currentWorld, NetPeer peer, NetCharacterBody3D obj, NetBuffer buffer, int maxBytes)
        {
        ...
        public static void OnPeerAcknowledge(NetCharacterBody3D obj, UUID peerId, int tick)
        {
            // No-op: NetNode3D doesn't track per-peer state
        }

        public static void OnPeerDisconnected(NetCharacterBody3D obj, UUID peerId)
        {
            // No-op: NetNode3D doesn't track per-peer state
        }

        public static NetCharacterBody3D NetworkDeserialize(WorldRunner currentWorld, NetPeer peer, NetBuffer buffer, NetCharacterBody3D existing = null)
        {
            ...
            var node = currentWorld.GetNodeFromNetId(networkID)?.RawNode as NetCharacterBody3D;
            if (node == null)
            {
                return null;
            }
            if (staticChildId > 0)
            {
                node = node.GetNodeOrNull(Protocol.UnpackNode(node.SceneFilePath, staticChildId)) as NetCharacterBody3D;
            }
            return node;
        }
        ...
        public async Task<NetCharacterBody3D> BsonDeserialize(NetBsonContext context, byte[] bson)
        {
            return await BsonDeserialize(context, bson, this);
        }

        public static async Task<NetCharacterBody3D> BsonDeserialize(NetBsonContext context, byte[] bson, NetCharacterBody3D obj)
        {
