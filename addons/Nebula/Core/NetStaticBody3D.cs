...
namespace Nebula
{
    /**
		<summary>
		<see cref="StaticBody3D">StaticBody3D</see>, extended with Nebula networking capabilities.
		See <see cref="NetNode"/> for more information.
		</summary>
	*/
    public partial class NetStaticBody3D : StaticBody3D, INetNode<NetStaticBody3D>, INotifyPropertyChanged
    {
        public NetworkController Network { get; internal set; }
        public NetStaticBody3D()
        {
        ...
        public static bool NetworkSerialize(WorldRunner currentWorld, NetPeer peer, NetStaticBody3D obj, NetBuffer buffer, int maxBytes)
        {
        ...
        public static void OnPeerAcknowledge(NetStaticBody3D obj, UUID peerId, int tick)
        {
            // No-op: NetNode3D doesn't track per-peer state
        }

        public static void OnPeerDisconnected(NetStaticBody3D obj, UUID peerId)
        {
            // No-op: NetNode3D doesn't track per-peer state
        }

        public static NetStaticBody3D NetworkDeserialize(WorldRunner currentWorld, NetPeer peer, NetBuffer buffer, NetStaticBody3D existing = null)
        {
            ...
            var node = currentWorld.GetNodeFromNetId(networkID)?.RawNode as NetStaticBody3D;
            if (node == null)
            {
                return null;
            }
            if (staticChildId > 0)
            {
                node = node.GetNodeOrNull(Protocol.UnpackNode(node.SceneFilePath, staticChildId)) as NetStaticBody3D;
            }
            return node;
        }
        ...
        public async Task<NetStaticBody3D> BsonDeserialize(NetBsonContext context, byte[] bson)
        {
            return await BsonDeserialize(context, bson, this);
        }

        public static async Task<NetStaticBody3D> BsonDeserialize(NetBsonContext context, byte[] bson, NetStaticBody3D obj)
        {
