...
namespace Nebula
{
    /**
		<summary>
		<see cref="RigidBody3D">RigidBody3D</see>, extended with Nebula networking capabilities.
		See <see cref="NetNode"/> for more information.
		</summary>
	*/
    public partial class NetRigidBody3D : RigidBody3D, INetNode<NetRigidBody3D>, INotifyPropertyChanged
    {
        public NetworkController Network { get; internal set; }
        public NetRigidBody3D()
        {
        ...
        public static void OnPeerDisconnected(NetRigidBody3D obj, UUID peerId)
        {
            // No-op: NetNode3D doesn't track per-peer state
        }

        public static NetRigidBody3D NetworkDeserialize(WorldRunner currentWorld, NetPeer peer, NetBuffer buffer, NetRigidBody3D existing = null)
        {
            ...
            var node = currentWorld.GetNodeFromNetId(networkID)?.RawNode as NetRigidBody3D;
            if (node == null)
            {
                return null;
            }
            if (staticChildId > 0)
            {
                node = node.GetNodeOrNull(Protocol.UnpackNode(node.SceneFilePath, staticChildId)) as NetRigidBody3D;
            }
            return node;
        }
        ...
        public async Task<NetRigidBody3D> BsonDeserialize(NetBsonContext context, byte[] bson)
        {
            return await BsonDeserialize(context, bson, this);
        }

        public static async Task<NetRigidBody3D> BsonDeserialize(NetBsonContext context, byte[] bson, NetRigidBody3D obj)
        {
